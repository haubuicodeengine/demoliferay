package com.codeengine.liferay;
...
public class StudentPortlet extends MVCPortlet {
	...
	public void addOrUpdateStudent(ActionRequest request,
		...
		try {
			if (studentId > 0) {
				...
				StudentLocalServiceUtil.addStudentManual(name, email, serviceContext);
	...
	public void deleteStudent(ActionRequest request, ActionResponse response) {
		...
		try {
			ServiceContext serviceContext = ServiceContextFactory.getInstance(
					Student.class.getName(), request);			
			response.setRenderParameter("studentId", Long.toString(studentId));
